...
export default function TourDashboard() {
  const { user } = useAuth();
  const { items: bookings, loading: bookingsLoading } = useManager("tour-bookings");
  ...
  const bookingsTrend = tourStats.bookingsTrend || [
    { label: "Nov", value: 82 },
    { label: "Dec", value: 110 },
    { label: "Jan", value: 95 },
    { label: "Feb", value: 128 },
  ...
  const packages = tourStats.packages || [];
  ...
  return (
    <div className="space-y-6">
      <DashboardHero
        ...
        date={today}
        actions={[
          { label: dashLoading ? "Refreshing..." : "Refresh Tours", icon: "arrow-right", onClick: refresh },
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        <StatCard label="Total Tour Packages" value={String(tourStats.totalPackages || 5)} icon="ticket" tone="green" spark={[1, 2, 3, 4, 5, 5]} className={`animate-rise ${DELAYS[0]}`} />
        <StatCard label="Active Tours" value={String(tourStats.activeTours || 4)} icon="compass" tone="green" delta="+2" spark={[2, 3, 3, 4, 4, 4]} className={`animate-rise ${DELAYS[1]}`} />
        <StatCard label="Upcoming Bookings" value={String(tourStats.upcomingBookings || 128)} icon="calendar" tone="gold" delta="+12%" spark={[88, 95, 102, 110, 118, 128]} className={`animate-rise ${DELAYS[2]}`} />
        <StatCard label="Total Revenue" value={tourStats.totalRevenue || "$48.2k"} icon="trending-up" tone="gold" delta="+8%" spark={[32, 36, 39, 42, 46, 48]} className={`animate-rise ${DELAYS[3]}`} />
        <StatCard label="Available Guides" value={String(tourStats.availableGuides || 3)} icon="user" tone="green" spark={[1, 2, 2, 3, 3, 3]} className={`animate-rise ${DELAYS[4]}`} />
      </div>
      ...
      <ChartCard title="Tour Bookings Overview" subtitle="Bookings live trend" className="animate-rise delay-200">
      ...
      <div className="animate-rise delay-200">
        ...
        <DataTable columns={buildColumns(entityMeta("tour-bookings").columns)} rows={bookings.slice(0, 5)} loading={bookingsLoading} searchable={false} />
      </div>
