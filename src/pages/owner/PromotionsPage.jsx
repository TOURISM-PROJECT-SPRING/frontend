...
const discountText = (p) =>
  norm(p.discountType) === "PERCENT" ? `${p.discountValue}% OFF` : `$${p.discountValue} OFF`;
...
export default function OwnerPromotionsPage() {
  ...
  const [ownerHotelIds, setOwnerHotelIds] = useState([]);
  const [ownerHasHotels, setOwnerHasHotels] = useState(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [promoData, hotelData] = await Promise.all([
          managementService.getPromotions(),
          hotelService.getAllHotels(),
        ]);
        const myHotels = (hotelData || []).filter((h) => h.ownerId === user?.id);
        setOwnerHasHotels(myHotels.length > 0);
        setOwnerHotelIds(myHotels.map((h) => h.id));
  ...
  const scoped =
    ownerHasHotels === false
      ? promotions
      : ownerHotelIds.length
        ? promotions.filter((p) => ownerHotelIds.includes(p.hotelId))
        : [];

  const statuses = [...new Set(scoped.map((p) => p.status).filter(Boolean))];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Discount codes and special offers for your properties</p>
      </div>
      ...
      {scoped.length === 0 ? (
        <div className="text-center py-12 text-sm text-gray-400 dark:text-gray-500">No promotions for your properties yet.</div>
      ) : (
