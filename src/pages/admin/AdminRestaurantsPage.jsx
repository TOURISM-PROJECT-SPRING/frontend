...
const statusColors = { Active: "bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-400", Inactive: "bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 dark:text-gray-500" };

const initialRestaurants = [
  { id: 1, name: "Khmer Kitchen", location: "Siem Reap", cuisine: "Khmer", rating: 4.8, reviews: 234, priceRange: "$$", status: "Active", image: "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=400&h=300&fit=crop", phone: "+855 12 345 678" },
  { id: 2, name: "The Foreign Correspondents' Club", location: "Phnom Penh", cuisine: "Western", rating: 4.5, reviews: 189, priceRange: "$$$", status: "Active", image: "https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=400&h=300&fit=crop", phone: "+855 23 456 789" },
  { id: 3, name: "Bamboo Garden", location: "Battambang", cuisine: "Asian", rating: 4.3, reviews: 156, priceRange: "$", status: "Active", image: "https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=400&h=300&fit=crop", phone: "+855 53 234 567" },
  { id: 4, name: "Riverside Fusion", location: "Kampot", cuisine: "Fusion", rating: 4.6, reviews: 98, priceRange: "$$", status: "Active", image: "https://images.unsplash.com/photo-1559339352-11d035aa65de?w=400&h=300&fit=crop", phone: "+855 16 789 012" },
  { id: 5, name: "Ocean Catch", location: "Sihanoukville", cuisine: "Seafood", rating: 4.4, reviews: 211, priceRange: "$$", status: "Inactive", image: "https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=400&h=300&fit=crop", phone: "+855 34 567 890" },
  { id: 6, name: "Street Food Heaven", location: "Phnom Penh", cuisine: "Khmer", rating: 4.2, reviews: 312, priceRange: "$", status: "Active", image: "https://images.unsplash.com/photo-1526318896980-cf78c088247c?w=400&h=300&fit=crop", phone: "+855 12 876 543" },
];
...
export default function AdminRestaurantsPage() {
  ...
  const handleSave = (data) => {
    if (editItem) {
      setRestaurants((prev) => prev.map((r) => (r.id === editItem.id ? { ...r, ...data } : r)));
    } else {
      setRestaurants((prev) => [{ ...data, id: Date.now(), rating: 0, reviews: 0, image: "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=400&h=300&fit=crop" }, ...prev]);
    }
    setFormOpen(false);
    setEditItem(null);
  };
  ...
  return (
    <div className="space-y-6">
        ...
        {filtered.map((r) => (
          <div key={r.id} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 overflow-hidden hover:shadow-md transition">
            <div className="relative h-40">
              ...
              <span className={`absolute top-3 left-3 px-2.5 py-1 rounded-full text-[10px] font-bold ${statusColors[r.status]}`}>{r.status}</span>
            </div>
            <div className="p-4">
              <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-1">{r.name}</h3>
              <p className="text-[11px] text-gray-400 dark:text-gray-500 flex items-center gap-1 mb-2"><MapPin className="w-3 h-3" /> {r.location}</p>
              <div className="flex items-center gap-3 mb-3">
                <span className="px-2 py-0.5 bg-primary/10 text-primary text-[10px] font-bold rounded-full">{r.cuisine}</span>
                <span className="text-[11px] text-gray-400 dark:text-gray-500">{r.priceRange}</span>
                <span className="flex items-center gap-0.5 text-[11px] text-yellow-500"><Star className="w-3 h-3 fill-yellow-400" /> {r.rating}</span>
                <span className="text-[11px] text-gray-400 dark:text-gray-500">({r.reviews})</span>
              </div>
              <div className="flex items-center justify-between pt-3 border-t border-gray-50 dark:border-gray-800">
                <span className="text-[11px] text-gray-400 dark:text-gray-500">{r.phone}</span>
                <div className="flex items-center gap-1">
                  <button onClick={() => setViewTarget(r)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 dark:bg-gray-700 transition"><Eye className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500" /></button>
      ...
      {filtered.length === 0 && <div className="text-center py-12 text-sm text-gray-400 dark:text-gray-500">No restaurants found.</div>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div className="p-6">
              ...
              <p className="text-sm text-gray-400 dark:text-gray-500 flex items-center gap-1"><MapPin className="w-3.5 h-3.5" /> {viewTarget.location}</p>
              <div className="flex items-center gap-3 mt-3">
                <span className="px-2 py-0.5 bg-primary/10 text-primary text-[10px] font-bold rounded-full">{viewTarget.cuisine}</span>
                <span className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">{viewTarget.priceRange}</span>
                ...
                <span className="text-sm text-gray-400 dark:text-gray-500">({viewTarget.reviews} reviews)</span>
              </div>
              <div className="mt-4 space-y-2">
                <div className="flex justify-between text-sm"><span className="text-gray-400 dark:text-gray-500">Phone</span><span className="text-gray-600 dark:text-gray-300">{viewTarget.phone}</span></div>
              </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); const d = Object.fromEntries(new FormData(e.target)); handleSave({ ...editItem, ...d, id: editItem?.id || Date.now() }); }} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="location" required defaultValue={editItem?.location || ""} className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-primary transition" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Cuisine *</label>
                  <select name="cuisine" required defaultValue={editItem?.cuisine || ""} className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-primary transition">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="phone" required defaultValue={editItem?.phone || ""} className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-primary transition" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Price Range *</label>
                  <select name="priceRange" required defaultValue={editItem?.priceRange || ""} className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-primary transition">
