...
export default function MobileFilterDrawer({ open, onClose, filters, onPatch, resultCount }) {
  return (
    <Drawer open={open} onClose={onClose} title="Filters" width="max-w-sm">
      <div className="flex flex-col gap-4 pb-24">
        <FilterSidebar filters={filters} onPatch={onPatch} />
      </div>
      <div className="sticky bottom-0 -mx-5 mt-4 border-t border-line bg-white px-5 py-3">
        ...
        >
          Show {resultCount.toLocaleString("en-US")} {resultCount === 1 ? "stay" : "stays"}
        </button>
      </div>
    </Drawer>
  );
