  ...
  return (
    <div className="flex items-center justify-between rounded-xl border border-line bg-canvas px-3 py-2">
      ...
      <div className="flex items-center gap-2">
        <button type="button" onClick={() => step(-1)} aria-label={t("tourModal.decrease", { label })} className="grid h-7 w-7 place-items-center rounded-lg border border-line text-brand-700 hover:bg-brand-50">
          <Icon name="minus" size={14} />
        </button>
        <span className="min-w-[2ch] text-center text-sm font-bold text-brand-800">{value}</span>
        <button type="button" onClick={() => step(1)} aria-label={t("tourModal.increase", { label })} className="grid h-7 w-7 place-items-center rounded-lg bg-brand-700 text-white hover:bg-brand-800">
    ...
    <Modal open={open} onClose={onClose} size="xl">
      <div className="overflow-hidden rounded-t-2xl bg-gradient-to-br from-brand-700 to-brand-900 p-6 text-white">
        <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-gold-300">
          {tour.category || t("cart.kindTour")} {tour.status ? ` · ${tour.status}` : ""}
        </p>
        ...
        <p className="mt-1.5 flex items-center gap-1.5 text-sm text-white/80">
          ...
          {tour.province || tour.address || tour.location || t("common.cambodia")}
          {tour.duration && (
      ...
      <div className="space-y-6">
        ...
        {tour.description && (
          <div>
            <h3 className="font-display text-lg font-bold text-brand-800">{t("tourModal.about")}</h3>
        ...
        {tour.itinerary?.length > 0 && (
          <div>
            <h3 className="font-display text-lg font-bold text-brand-800">{t("tourModal.itinerary")}</h3>
        ...
        {tour.includes?.length > 0 && (
          <div>
            <h3 className="font-display text-lg font-bold text-brand-800">{t("tourModal.included")}</h3>
        ...
        <div>
          <h3 className="font-display text-lg font-bold text-brand-800">{t("tourModal.guide")}</h3>
          <div className="mt-3">
      ...
      <div className="mt-6 rounded-2xl border border-gold-300 bg-gold-50 p-5">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wide text-gold-700">{t("tourModal.tripTotal")}</p>
            <p className="font-display text-2xl font-bold text-brand-800">
              {price ? `${money(total)}` : t("tourModal.onRequest")}
              {price && <span className="text-xs font-medium text-muted"> {t("tourModal.forGuests", { count: guests })}</span>}
            </p>
          </div>
          <div className="space-y-2">
            ...
            <div className="w-44">
              <Meter label={t("tourModal.guestsLabel")} value={guests} step={(d) => setGuests((g) => Math.min(12, Math.max(1, g + d)))} />
            </div>
          </div>
        </div>
        <button
        ...
        >
          <Icon name="plus" size={18} /> {t("tourModal.addToTrip")}{price ? ` · ${money(total)}` : ""}
        </button>
        <p className="mt-3 flex items-center gap-1.5 text-[11px] font-medium text-gold-700">
          <Icon name="check-circle" size={13} /> {t("tourModal.perks")}
        </p>
      </div>
    </Modal>
  );
