          ...
          <thead>
            <tr className="border-b border-gray-50 dark:border-gray-800">
              {["ID", "Guest", "Property", "Date", "Amount", "Status"].map((h) => (
                <th key={h} className={`text-left text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider px-4 py-2.5 ${h === "Property" ? "hidden lg:table-cell" : ""} ${h === "Date" ? "hidden md:table-cell" : ""}`}>
                  {h}
                </th>
              ))}
            </tr>
              ...
              <tr key={b.id} className="border-b border-gray-50 dark:border-gray-800 last:border-0 hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition">
                <td className="px-4 py-2.5 text-[13px] font-medium text-gray-900 dark:text-white">
                  <span className="inline-flex items-center gap-1.5">
                    {b.id}
                    <span className="text-[9px] font-semibold text-primary/70 bg-primary/5 px-1.5 py-0.5 rounded">{b.type}</span>
                  </span>
                </td>
